{config, ...}: {
  services = let
    vhost = {ssl}: {
      ...
      extraConfig = ''
        ...
        ssl_dhparam /run/secrets/plex/dhparam.pam;
  ...
  in {
    plex = {
      enable = true;
      dataDir = "/data/plex";
      ## At least, for initial setup, firewall needs to be open.
      ## To install Plex it's required to do it while skipping nginx proxy
      ## through <hostIP>:32400
      openFirewall = true;
    };
    nginx = {
      appendHttpConfig = ''
      ...
      '';
      upstreams.plex_backend = {
        extraConfig = ''
          keepalive 32;
        '';
        servers."127.0.0.1:32400" = {};
      };
      virtualHosts."plex.xxxx.net" = vhost {ssl = true;};
      virtualHosts."plex.local" = vhost {ssl = false;};
    };
  };
  sops.secrets."plex/dhparam.pam" = {
    format = "binary";
